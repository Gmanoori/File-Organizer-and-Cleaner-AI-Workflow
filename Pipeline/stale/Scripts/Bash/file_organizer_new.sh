...
Functions:
  ...
  ${CYAN}move_extracted_files()${NC} Moves files from an extracted archive into root target folders
...
# Recursively scans an extracted archive folder and moves recognized files
# (CSV, XLSX, XLS, JSON) into the root target directories — the same CSV/,
# XLSX/, XLS/, JSON/ folders used by the main move_files() pass.
move_extracted_files() {
    local src_dir="$1"
    local moved_count=0

    log_info "Organizing extracted contents of: ${CYAN}$(basename "$src_dir")${NC}"
    ...
    while IFS= read -r -d '' file; do
        ...
        case "$extension" in
            ...
            json)
                ...
                ;;
            *)
                log_verbose "Skipping unsupported type in extracted archive: $filename"
                ;;
        esac
    done < <(find "$src_dir" -type f -print0)

    log_success "Files organized from archive: ${GREEN}${moved_count}${NC}"
}
